...
artifacts/state.yaml = 현재 phase와 활성 artifact를 가리키는 상태 포인터
```
...
이미지 생성은 GPT Image 2를 기본으로 가정합니다. GPT Image 2는 OpenAI의 이미지 생성/편집 모델이며 `gpt-image-2` 모델 문서와 이미지 생성 API 문서가 근거입니다. 현재 Codex 환경에서 GPT Image 2를 사용할 수 없으면 다른 모델로 조용히 대체하지 않고 blocked manifest를 반환합니다.
...
| GPT Image 2 unavailable 시 blocked manifest 반환 | 요구사항이 GPT Image 2 사용이므로 다른 모델 대체는 계약 위반입니다. | GPT Image 2 모델 문서와 이미지 생성 가이드는 `gpt-image-2`가 이미지 생성/편집 모델임을 설명합니다. |
...
flowchart TD
    ...
    B --> C["탐색 축 3-5개 정의"]
    C --> D["GPT Image 2 reference candidates 생성"]
...
flowchart TD
    ...
    A --> F["GPT Image 2 mention check"]
    A --> G["active_feedback_packet check"]
    A --> H[".DS_Store absence"]
```

### 왜 검증 스크립트를 두나

문서형 pressure test만 있으면 매번 사람이 규칙을 해석해야 합니다. 정적 검증 스크립트는 최소 구조가 깨졌는지 빠르게 확인합니다. 다만 실제 `$curate`, `$craft` discovery와 GPT Image 2 호출 가능 여부는 fresh Codex session에서 따로 확인해야 합니다.
...
stateDiagram-v2
    ...
    Exploring --> Blocked: GPT Image 2 unavailable
    Crafting --> Blocked: GPT Image 2 unavailable
```
...
- [GPT Image 2 model page](https://developers.openai.com/api/docs/models/gpt-image-2): GPT Image 2 모델 성격과 endpoint 정보.
- [OpenAI Image generation guide](https://platform.openai.com/docs/guides/image-generation): 이미지 생성/편집 API와 GPT Image 계열 사용 방식.
